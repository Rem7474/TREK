...
| `LLM_TIMEOUT_MS`          | How long (in milliseconds) one AI-parsing call may take before it is abandoned — booking imports and receipt scans alike. Generous by default because a local vision model reading a photographed receipt on CPU routinely needs several minutes, and aborting throws the whole inference away. Nothing waits on it in the browser (scans run as a background job), so a long ceiling only costs a slow failure. Lower it if you use a cloud provider and would rather fail fast. Invalid values abort startup. | `900000` (15 min) |
