...
| `LLM_TIMEOUT_MS`          | How long (in milliseconds) one AI-parsing call may take before it is abandoned. Generous by default because a local vision model reading a photographed receipt on CPU routinely needs several minutes. Lower it if you use a cloud provider and would rather fail fast. Invalid values abort startup. | `900000` (15 min) |
