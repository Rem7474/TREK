...
export class LlmParseService {
  ...
  private async readReceiptLocally(
    ...
    // Both of these run under the one configured call ceiling (LLM_TIMEOUT_MS,
    // #2230). With reasoning off, a photo capped by capReceiptImage reads in
    // seconds to about two minutes on a CPU-only host; one the cap could not
    // decode goes whole and takes about four.
