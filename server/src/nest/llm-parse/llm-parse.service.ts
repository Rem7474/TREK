...
export class LlmParseService {
  ...
  private async readReceiptLocally(
    ...
    // extractEnforced gives up after five minutes. With reasoning off, a photo
    // capped by capReceiptImage reads in seconds to about two minutes on a
    // CPU-only host; one the cap could not decode goes whole and takes about
    // four, close enough to that ceiling that a slower host can lose it.
